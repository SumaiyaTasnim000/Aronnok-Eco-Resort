...
router.post("/", auth(["manager", "admin"]), async (req, res) => {
  try {
    ...
    const { edate, ename, eamount } = req.body;
    if (!edate || !ename || !eamount) {
      return res.status(400).json({ message: "Missing required fields" });
    }

    const expense = new Expense({
      eid: Date.now(), // ✅ always unique per insert
      edate,
      ename,
      eamount,
      ecreatedByUid: req.user?.uid || null,
      eisDeleted: false, // ✅ default to false
    });
...
router.get("/", auth(["manager", "admin"]), async (req, res) => {
  try {
    const expenses = await Expense.find({ eisDeleted: false }).sort({ edate: -1 });
    res.json(expenses);
  } catch (err) {
